...
ii solve(int cup, int cdown) {
	if(cup == endup) return mk(cup,cdown);

	int &r = dp[cup][cdown];
	if(r != -1) return mk(cup,cdown);
	
	ii best = mk(0,0);
	int ans = 0;
	for(int next:viz[citec[cup]]) {
		if(next >= cdown) {
			ii res = solve(cup+1,next+1);
			int at = dp[res.fi][res.sec] + value[cup] + value[next];
			if (at > ans) {
				ans = at;
				best = res;
				dp2[res.fi][res.sec]++;
	...
	ii x = solve(cup+1,cdown);
	if (dp[x.fi][x.sec] > ans) {
		ans = dp[x.fi][x.sec];
		dp2[x.fi][x.sec]++;
		best = x;
	}
	//int t = max(ans, solve(cup+1, cdown));
...
/*int recovery(int cup, int cdown) {
	if(cup == endup) return 0;
...
int main(){
	...
	while(tc--) {
		...
		ii x = solve(1,endup);
		cout <<dp[x.fi][x.sec] <<endl;
		cout <<dp2[x.fi][x.sec] <<endl;
